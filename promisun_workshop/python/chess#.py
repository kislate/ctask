# 本关任务：编写能命令行交互的井字棋游戏，要求电脑和玩家依次落子，
# 其中电脑每次落子时选择最优落子位置落子。
...
# 搜索树
# 以井字棋游戏为例应用搜索树
#    下图为棋盘,数字代表每一格的编号(与小键盘数字编号一致)
 
#     假设电脑先走, 其中圆圈为电脑落子位置,叉号为玩家落子位置, 经过电脑和玩家的数次落子,
#     棋盘状态如下所示,此时轮到电脑落子
 
#     电脑有三个落子位置可以选择,落子后棋盘状态如下
 
#     此时,对电脑来说三种棋盘状态将会导致不同的结局,后两种落子方式在下一步玩家落子后,电脑就输了,如图所示
 
#     因此,电脑应该选择对其更有利的第一种落子方式落子。反映在计算机程序中,则需要根据不同的棋盘状态来计算不同的价值,
#     电脑选择价值最高的位置落子。
#     电脑在搜索树中搜索时,需要假设玩家也非常聪明,因此在模拟玩家落子时,也需要选择对玩家最有利的位置进行落子,
#     直到双方下满棋盘(平局)或者一方胜出,得到对应落子位置的价值,从而选择价值最高的位置进行落子即可。
